...
if [ ! -d packages/dnx-mono ]; then
    mono .nuget/nuget.exe install dnx-mono -Version 1.0.0-rc1-final -ExcludeVersion -Out packages -Source https://www.myget.org/F/aspnetmaster/api/v3/index.json
    chmod a+x packages/dnx-mono/bin/dnu
    chmod a+x packages/dnx-mono/bin/dnx
fi
if [ ! -d packages/dnx-coreclr-linux-x64 ]; then
    mono .nuget/nuget.exe install dnx-coreclr-linux-x64 -Version 1.0.0-rc1-final -ExcludeVersion -Out packages -Source https://www.myget.org/F/aspnetmaster/api/v3/index.json
